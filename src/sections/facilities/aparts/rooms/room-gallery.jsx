...
export default function RoomGallerySection() {
  ...
  useEffect(() => {
    if ((params.id > 0 && loading) || lineChangeLoading || isEdit)
      GetPhotosRoom(params.id).then((res) => { setPhoto(res.data); setPhotoList(res.data); setLoading(false); setLineChangeLoading(false); setIsEdit(false) })
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12}>
            <MainCard>
              ...
              <PhotoModal room={true} open={photoModal} modalToggler={setPhotoModal} villaId={params.id} setIsEdit={setIsEdit} lastLine={photo[photoList.length - 1]?.attributes.line} setLoading={setLoading} />
