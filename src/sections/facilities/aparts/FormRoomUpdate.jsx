...
const getInitialValues = ({ data }) => {
    const newPriceDate = {
        ...
        video: data?.attributes?.electricityMeterNumber || "",
        waterMaterNumber: data?.attributes?.waterMaterNumber || "",
        electricityMeterNumber: data?.attributes?.electricityMeterNumber || "",
        internetMeterNumber: data?.attributes?.internetMeterNumber || "",
        wifiPassword: data?.attributes?.wifiPassword || "",
        person: data?.attributes?.person || 0,
        bath: data?.attributes?.bath || 0,
        room: data?.attributes?.room || 0
    };
    return newPriceDate;
};
...
export default function FormRoomUpdate({ closeModal }) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting }) => {
            try {
                // formik.values.roomId = { connect: [params.id] };
                ...
                await RoomChangeState(params?.id, data).then((res) => {
                    if (!res?.error) {
                        openSnackbar({
                            open: true,
